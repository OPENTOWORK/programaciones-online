...
import {
  ...
  summarizeYear,
  toWorkoutRows,
} from './lib/calisteniaYearProgram.mjs';
import {
  annualRateId,
  resolveDatabaseUrl,
  resolveProgramId,
...
function parseArgs(argv) {
  ...
  return {
    program: valueOf('--program', 'Calistenia'),
...
function printWeekPreview(days, week) {
  ...
  for (const day of days.filter((entry) => entry.week === week)) {
    console.log(`\n${day.weekdayLabel} · ${day.dayTypeName} · ${day.date}`);
    for (const session of day.sessions) {
      ...
      if (session.core) {
        for (const line of session.core.split('\n')) {
          console.log(`    ${line}`);
        }
      }
      if (session.cooldown) console.log(`    Vuelta a la calma: ${session.cooldown}`);
    }
  }
}
...
/**
 * Rango de identificadores que la importación anual de AimHarder usa para un track, por si hace
 * falta retirar solo esos entrenos sin vaciar el programa completo.
 */
function aimharderAnnualRange(programName) {
  const trackKey = resolveTrackKey(programName);
  if (!trackKey) return null;
  return { from: annualRateId('2000-12-31', trackKey), to: annualRateId('2000-01-01', trackKey) };
}

/**
 * Cada parte es un bloque `do $$` autosuficiente y los insert usan `on conflict`, así que se pueden
 * pegar en cualquier orden y repetir. La limpieza previa solo va en la primera.
 */
function buildSqlFile(rows, { program, programId, replaceProgram }, { part = 1, totalParts = 1 } = {}) {
  const aimharderRange = aimharderAnnualRange(program);
  const isFirstPart = part === 1;
  const lines = [
    `-- Programación anual de ${program} generada por scripts/generate-calistenia-year.mjs`,
    totalParts > 1
      ? `-- Parte ${part} de ${totalParts} · ${rows.length} sesiones. Pega las partes en orden en el editor SQL de Supabase.`
      : '-- Pégalo en el editor SQL de Supabase si no tienes DATABASE_URL a mano.',
    isFirstPart
      ? replaceProgram
        ? '-- Borra TODOS los entrenos del programa antes de insertar (incluidos los importados de AimHarder).'
    ...
    ...(!isFirstPart || replaceProgram || !aimharderRange
      ? []
      : [
          '--',
          `-- Si quieres retirar además los entrenos anuales que la importación de AimHarder dejó en ${program},`,
          ...
          `--     and aimharder_rate_id between ${aimharderRange.from} and ${aimharderRange.to};`,
        ]),
    'do $$',
    'declare',
    '  target_program uuid;',
...
async function listProgramCandidates(client, programName) {
  const result = await client.query(
    `select p.id, p.name, count(e.id)::int as sessions
     from public.programas p
     left join public.entrenos_diarios e on e.program_id = p.id
     where lower(trim(translate(p.name, 'áéíóúÁÉÍÓÚ', 'aeiouAEIOU')))
           like '%' || lower(trim(translate($1, 'áéíóúÁÉÍÓÚ', 'aeiouAEIOU'))) || '%'
...
async function applyToDatabase(rows, { program, programId, clean, replaceProgram }) {
  const databaseUrl = resolveDatabaseUrl(PROJECT_REF);
  if (!databaseUrl) {
    throw new Error(
      'Falta DATABASE_URL (o SUPABASE_DB_PASSWORD) en .env. Usa --sql para generar el script y pegarlo en Supabase.',
    );
  }
  ...
  try {
    let resolvedId = programId;

    if (!resolvedId) {
      const candidates = await listProgramCandidates(client, program);

      if (candidates.length > 1) {
        const detail = candidates
          .map((row) => `   · ${row.name} → ${row.id} (${row.sessions} sesiones)`)
          .join('\n');
        throw new Error(
          `Hay ${candidates.length} programas que coinciden con "${program}".\n${detail}\n` +
            'Relanza el comando con --program-id <id> para elegir el correcto.',
        );
      }

      resolvedId = candidates[0]?.id ?? (await resolveProgramId(client, program));
    }

    if (!resolvedId) {
      throw new Error(`No se encontró el programa "${program}" en Supabase`);
    }
    ...
    console.log(`✓ ${rows.length} sesiones guardadas en el programa ${program} (${resolvedId})`);
    ...
    console.log(`✓ Verificado en Supabase: ${sessions} sesiones repartidas en ${days} días`);
  } catch (error) {
...
async function main() {
  ...
  const days = buildCalisteniaYear({ year: options.year, restDays: options.restDays });
  ...
  console.log(
    `Programación de ${options.program} · año natural ${options.year}: ${weeks} semanas · ${trainingDays} días de entreno · ${restDays} de descanso · ${rows.length} sesiones`,
  );
  console.log(
    `Fechas: ${rows[0].workoutDate} → ${rows.at(-1).workoutDate} (entreno lunes, martes, jueves, viernes y sábado)`,
  );
  ...
  if (!options.apply) {
    console.log('\nNada escrito en Supabase. Añade --apply para guardarlo (o --sql para el script).');
    return;
  }
