...
import {
  ...
  summarizeYear,
  toWorkoutRows,
} from './lib/athxYearProgram.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const ROOT_DIR = join(__dirname, '..');
const PROJECT_REF = 'nsdurlikkuoxqobabixr';
const DEFAULT_PROGRAM_ID = '9b9d3120-adca-4227-aa0d-22fd5f617e61';
...
function parseArgs(argv) {
  ...
  return {
    ...
    programId: valueOf('--program-id', DEFAULT_PROGRAM_ID),
    year,
...
function printWeekPreview(days, week) {
  ...
  for (const day of days.filter((entry) => entry.week === week)) {
    ...
    for (const session of day.sessions) {
      console.log(`  [${session.kind}] ${session.name} · ${session.estimatedDuration}`);
    }
  }
}
...
function buildSqlFile(rows, { program, programId, replaceProgram }, { part = 1, totalParts = 1 } = {}) {
  ...
  const lines = [
    `-- Programación anual de ${program} · scripts/generate-athx-year.mjs`,
    'do $$',
    'declare',
    '  target_program uuid;',
    'begin',
...
function writeSqlFiles(rows, options) {
  ...
  for (let part = 1; part <= totalParts; part += 1) {
    ...
    written.push({ path: target, rows: chunk.length });
  }
...
async function applyToDatabase(rows, options) {
  ...
  let resolvedId = options.programId;
  if (!resolvedId) {
    resolvedId = await resolveProgramId(client, options.program);
  }
  if (!resolvedId) {
  ...
  try {
    ...
    if (options.replaceProgram) {
      ...
      console.log(`· Vaciado ATHX: ${deleted.rowCount} sesiones eliminadas`);
    ...
    console.log(`✓ ${rows.length} sesiones guardadas en ATHX (${resolvedId})`);
    ...
    console.log(`✓ Verificado: ${check.rows[0].sessions} sesiones en ${check.rows[0].days} días`);
  } catch (error) {
...
async function main() {
  ...
  const days = buildAthxYear({ year: options.year, restDays: options.restDays });
  ...
  console.log(
    `ATHX · año ${options.year}: ${weeks} semanas · ${trainingDays} días de entreno · ${restDayCount} de descanso · ${rows.length} sesiones`,
  );
  console.log('Entreno lun · mié · jue · sáb · descanso mar · vie · dom');
  ...
  if (options.sql) {
    for (const file of writeSqlFiles(rows, options)) {
      console.log(`✓ SQL → ${file.path} (${file.rows} sesiones)`);
    }
  }

  if (!options.apply) {
    console.log('\nAñade --apply --replace-program para publicar en Supabase.');
    return;
  }
