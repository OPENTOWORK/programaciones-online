...
function block(label, { title, timing, note, items = [] } = {}) {
...
function splitAlternates(body) {
  return body.split(/\s+O\s+/i).map((part) => part.trim()).filter(Boolean);
}

function inferRepsAndDetail(prescription) {
  const complex =
...
function formatExerciseName(category, detail, isAlternate) {
  const prefix = isAlternate ? '(Alt) ' : '';
  if (!detail) return `${prefix}${category}`;
  return `${prefix}${category} — ${detail}`;
}

function formatExerciseItem(exercise) {
  const name =
...
function parseVariant(category, text, isAlternate) {
...
function parseBalanceSegment(body) {
...
function parseStrengthSegment(segment) {
