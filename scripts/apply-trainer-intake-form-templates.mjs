...
async function main() {
  ...
  if (!databaseUrl) {
    throw new Error(
      'Falta DATABASE_URL (o SUPABASE_DB_PASSWORD) en .env para ejecutar la migración SQL',
    );
  }

  const sql = readFileSync(
    join(__dirname, '..', 'supabase', 'trainer-intake-form-templates.sql'),
    'utf8',
  );
  ...
  console.log('✓ Tablas trainer_intake_form_templates y athlete_intake_submissions aplicadas');
}
