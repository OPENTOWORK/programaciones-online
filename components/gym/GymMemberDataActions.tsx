...
import {
  exportGymMembers,
  pickAndParseGymMembers,
  type GymMemberDataFormat,
  type GymMemberImportRow,
} from '@/lib/gymMemberDataTransfer';
import type { GymMember } from '@/lib/gymTypes';

type MenuKind = 'import' | 'export' | null;

interface GymMemberDataActionsProps {
  gymId: string;
  members: GymMember[];
  fileStem: string;
  disabled?: boolean;
  onImport: (rows: GymMemberImportRow[]) => Promise<{
    created: number;
    updated: number;
    failed: number;
    skipped: number;
...
export function GymMemberDataActions({
  gymId,
  members,
  fileStem,
  disabled = false,
  ...
  onNotice,
  onError,
}: GymMemberDataActionsProps) {
  const [menu, setMenu] = useState<{ kind: MenuKind; anchor: PopoverAnchor } | null>(null);
  const [busy, setBusy] = useState(false);

  const closeMenu = () => setMenu(null);

  const handleExport = async (format: GymMemberDataFormat) => {
    closeMenu();
    if (members.length === 0) {
      onError('No hay miembros para exportar con el filtro actual.');
      return;
    }

    setBusy(true);
    const result = await exportGymMembers(gymId, members, format, fileStem);
    setBusy(false);
    if (result.error) onError(result.error);
    else onNotice(`Exportados ${result.data ?? members.length} contactos en ${format === 'csv' ? 'CSV' : 'Excel'}.`);
  };

  const handleImport = async (format: GymMemberDataFormat) => {
    closeMenu();
    setBusy(true);

    const picked = await pickAndParseGymMembers(format);
    ...
    onNotice(
      `Importación completada: ${summary.created} creados` +
        ...
        (summary.memberships ? `, ${summary.memberships} tarifas asignadas` : '') +
  ...
  return (
    <View style={styles.wrap}>
      ...
      <PopoverMenu
        ...
        title={menu?.kind === 'export' ? 'Exportar contactos como' : menu?.kind === 'import' ? 'Importar desde' : undefined}
        actions={menuActions}
        onClose={closeMenu}
      />
    </View>
