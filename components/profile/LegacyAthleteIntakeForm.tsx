...
export function LegacyAthleteIntakeForm({ returnTo }: { returnTo?: string }) {
  const router = useRouter();
  const { form, saving, save } = useAthleteIntakeForm();
  ...
  return (
    <ScreenWrapper>
      ...
      <Card style={styles.section}>
        <SectionHeader title="Objetivos de entrenamiento" subtitle="Puedes elegir más de uno" />
      ...
      <Card style={styles.section}>
        ...
        <Input
          ...
          placeholder="Cuéntanos tu recorrido..."
          value={experience}
        ...
        <Input label="¿De qué equipamiento deportivo dispones?" value={equipment} onChangeText={setEquipment} multiline />
        <Input label="¿Qué disponibilidad tienes para entrenar?" value={availability} onChangeText={setAvailability} multiline />
        <Input label="Flexiones" value={pushups} onChangeText={setPushups} keyboardType="numeric" />
        <Input label="Sentadillas" value={squats} onChangeText={setSquats} keyboardType="numeric" />
        <Input label="Dominadas" value={pullups} onChangeText={setPullups} keyboardType="numeric" />
      </Card>

      <Card style={styles.section}>
        <SectionHeader title="Historial de salud y lesiones" />
        <Text style={styles.fieldLabel}>¿Tienes o has tenido alguna lesión?</Text>
        <YesNoChips value={hasInjuries} onChange={setHasInjuries} />
        <Input label="Detalle (si aplica)" value={injuriesDetail} onChangeText={setInjuriesDetail} multiline />
        <Text style={[styles.fieldLabel, styles.fieldLabelSpaced]}>¿Tomas medicación relevante?</Text>
        <YesNoChips value={takesMedication} onChange={setTakesMedication} />
        <Text style={[styles.fieldLabel, styles.fieldLabelSpaced]}>¿Has tenido cirugías relevantes?</Text>
        <YesNoChips value={hadSurgery} onChange={setHadSurgery} />
        <Text style={[styles.fieldLabel, styles.fieldLabelSpaced]}>¿Tienes alguna condición médica?</Text>
...
const styles = StyleSheet.create({
  ...
  optionChipSelected: { borderColor: colors.accent, backgroundColor: withAlpha(colors.accent, '22') },
